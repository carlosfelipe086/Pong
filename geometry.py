#   Intersecção entre 2 linhas, utilizado na detecção de colisões
#   Fonte: https://www.pygame.org/wiki/IntersectingLineDetection
    ...
    if p is not None:               
        ...
        for point in p:                 
            try:    
                ...
                print("point was invalid {0}".format(point))
...
def distPointToSegment(p1, p2, c): 
    x1,y1 = p1
    x2,y2 = p2
    x3,y3 = c
    px = x2-x1
    ...
    u =  ((x3 - x1) * px + (y3 - y1) * py) / float(lensqr)
